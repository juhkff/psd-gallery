...
| 验证 | 结果 |
|---|---|
| `npm run build`（manifest + tsc + vite build） | 通过；worker 独立 chunk 303 kB，主包 258 kB（gzip 81 kB） |
| `npm run test:unit` | 4 个文件 / 66 个用例通过 |
| `npm test:e2e`（真实 headless Chrome 驱动 dist/） | **18/18 通过** |
| `npm run verify:proxy`（临时 8.3 MP PSD） | 通过：3520×2360 → 画布 1500×1006，5 层各解码一次，切换图层不再解码 |

e2e 覆盖：日期分组渲染、缩略图真实解码、打开作品列出 6 个图层、画布合成、
点击图层改变像素且 aria-pressed 翻转、再点一次像素完全还原、主线程最大帧间隔 17 ms、
**全部 3 个作品的下载 PSD 与源文件 md5 完全一致**、无页面报错；点击作品到首帧 27 ms。
